...
bool zvm_block_step_back(zvm_block_t *block){
    if(block->offset <= 0) return false;
    block->offset--;
    return true;
}

bool zvm_block_step_forward(zvm_block_t *block){
    if(block->offset >= (block->element_count * block->element_size) - 1) return false;
    block->offset++;
    return true;
}
